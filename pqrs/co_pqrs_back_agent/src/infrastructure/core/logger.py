...
correlation block (``cid``/``cust``/``req``) so a single conversation can be
traced end-to-end without grepping a file.

Discipline (to keep the terminal readable):
- INFO  -> milestones that describe the "path" the agent took.
- DEBUG -> detail: full payloads, prompts, full conversation/JSON dumps, tokens.
- WARNING -> recoverable issues (no back-data, fallback used, invalid input).
- ERROR -> something broke the turn.

Enable detail with ``AGENTEPQR_LOG_LEVEL=DEBUG``.
"""
...
_SERVICE_DEFAULT_NAME = "agentepqr"
_LEVEL_ENV_VAR = "AGENTEPQR_LOG_LEVEL"
_LOG_FORMAT = "%(asctime)s | %(levelname)s | %(name)s | %(corr)s | %(message)s"

_QUIET_LOGGERS = (
    "httpx",                                       # duplica opensearch_client
    "httpcore",
    "infrastructure.persistence.opensearch_client",  # duplica httpx
)
...
def configure_logging(level: int | str | None = None) -> None:
    ...
    quiet_level = os.getenv("QUIET_LOG_LEVEL", "WARNING")
