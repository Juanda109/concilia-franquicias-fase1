"""Fire-and-forget E2E trace emission for co_pqrs_back_data.

Pushes structured trace events (ASO / Postgres / external calls, success AND
failure) to the co_pqrs_back_error_handler service, which persists them to MinIO
near real-time (bucket ``audit-logs``, prefix ``traces/``).

Design constraints:
- **Never blocks and never raises.** Observability must not affect the request
  path or its latency. Any transport failure is swallowed (logged at debug).
- **Works from any context** (async loop, ``asyncio.to_thread`` worker, or a
  plain sync call). We therefore emit from a short-lived daemon thread doing a
  synchronous HTTP POST, instead of relying on a running event loop.
- **No-op when ``ERROR_HANDLER_SERVICE_URL`` is unset**, so it is safe by
  default and in tests.

Secrets (tsec, passwords) must be stripped by the caller before emitting.
"""
...
from infrastructure.core.logger import get_correlation_id, get_customer_id, get_logger
...
def _post_trace_event(base_url: str, payload: dict[str, Any]) -> None:
    """POST the trace event synchronously, swallowing all errors."""
...
def schedule_trace_event(
    *,
    ...
    cust_id = customer_id if customer_id is not None else _clean(get_customer_id())
